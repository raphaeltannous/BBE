...
static void
BBE_output_verse(const BBE_verse *verse, FILE *f, const BBE_config *config)
{
    fprintf(
        f,
        config->highlighting ?
...
static bool
BBE_output(const BBE_ref *ref, FILE *f, const BBE_config *config)
{
    BBE_next_data next = {
        .current = 0,
        .next_match = -1,
    ...
    BBE_verse *last_printed = NULL;
    for (int verse_id; (verse_id = BBE_next_verse(ref, config, &next)) != -1; ) {
        BBE_verse *verse = &BBE_verses[verse_id];
        BBE_book *book = &BBE_books[verse->book - 1];
        ...
        if (config->pretty) {
            ...
            BBE_output_verse(verse, f, config);
        } else {
            fprintf(
                f,
                config->highlighting ?
...
static int
BBE_render_pretty(const BBE_ref *ref, const BBE_config *config)
{
    int fds[2];
    if (pipe(fds) == -1) {
        return 1;
    ...
    bool printed = BBE_output(ref, output, config);
    if (!printed) {
        kill(pid, SIGTERM);
    }
    fclose(output);
...
int
BBE_render(const BBE_ref *ref, const BBE_config *config)
{
    if (config->pretty) {
        return BBE_render_pretty(ref, config);
    }
