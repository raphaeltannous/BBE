...
BBE_newref()
{
    return calloc(1, sizeof(BBE_ref));
}
...
static bool
BBE_bookequal(const char *a, const char *b, bool short_match)
{
    for (size_t i = 0, j = 0; ; ) {
...
static bool
BBE_book_matches(const BBE_book *book, const char *s)
{
    return BBE_bookequal(book->name, s, false) ||
        BBE_bookequal(book->abbr, s, false) ||
        BBE_bookequal(book->name, s, true);
}

static int
BBE_book_fromname(const char *s)
{
    for (int i = 0; i < BBE_books_length; i++) {
        const BBE_book *book = &BBE_books[i];
        if (BBE_book_matches(book, s)) {
            return book->number;
        }
    }
    return 0;
...
static int
BBE_scanbook(const char *s, int *n)
{
    int i;
    int mode = 0;
    for (i = 0; s[i]; i++) {
...
BBE_parseref(BBE_ref *ref, const char *ref_str)
{
    ...
    int n = 0;
    if (BBE_scanbook(ref_str, &n) == 1) {
        // 1, 2, 3, 3a, 4, 5, 6, 8, 9
        char *bookname = strndup(ref_str, n);
        ref->book = BBE_book_fromname(bookname);
        free(bookname);
        ref_str = &ref_str[n];
    } else if (ref_str[0] == '/') {
        // 7
