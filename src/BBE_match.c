...
static bool
BBE_verse_matches(const BBE_ref *ref, const BBE_verse *verse)
{
    switch (ref->type) {
        case KJV_REF_SEARCH:
...
static int
BBE_chapter_bounds(int i, int direction, int maximum_steps)
{
    assert(direction == KJV_DIRECTION_BEFORE || direction == KJV_DIRECTION_AFTER);

    int steps = 0;
    for ( ; 0 <= i && i < BBE_verses_length; i += direction) {
        bool step_limit = (maximum_steps != -1 && steps >= maximum_steps) ||
            (direction == KJV_DIRECTION_BEFORE && i == 0) ||
            (direction == KJV_DIRECTION_AFTER && i + 1 == BBE_verses_length);
        if (step_limit) {
            break;
        }

        const BBE_verse *current = &BBE_verses[i], *next = &BBE_verses[i + direction];
...
static int
BBE_next_match(const BBE_ref *ref, int i)
{
    for ( ; i < BBE_verses_length; i++) {
        const BBE_verse *verse = &BBE_verses[i];
        if (BBE_verse_matches(ref, verse)) {
            return i;
        }
    }
    return -1;
...
static void
BBE_next_addrange(BBE_next_data *next, BBE_range range) {
...
{
    if (next->current >= BBE_verses_length) {
        return -1;
    }

    if (next->matches[0].start != -1 && next->matches[0].end != -1 && next->current >= next->matches[0].end) {
        next->matches[0] = next->matches[1];
        next->matches[1] = (BBE_range){-1, -1};
    }

    if ((next->next_match == -1 || next->next_match < next->current) && next->next_match < BBE_verses_length) {
        int next_match = BBE_next_match(ref, next->current);
        if (next_match >= 0) {
            next->next_match = next_match;
            BBE_range bounds = {
                .start = BBE_chapter_bounds(next_match, KJV_DIRECTION_BEFORE, config->context_chapter ? -1 : config->context_before),
                .end = BBE_chapter_bounds(next_match, KJV_DIRECTION_AFTER, config->context_chapter ? -1 : config->context_after) + 1,
            };
            BBE_next_addrange(next, bounds);
        } else {
            next_match = BBE_verses_length;
        }
    }
